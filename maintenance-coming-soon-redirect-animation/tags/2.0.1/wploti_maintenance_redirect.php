...
if( !class_exists("wploti_maintenance_redirect") ) {
	...
	class wploti_maintenance_redirect {
			...
			foreach($headers as $header) {
				...
				switch ($wploti_header) {
					...
					case $header['code']:
						...
						$console = $header['code'] === '503' ? $header['title'] : $header['description'];
						$console_style = '';
		...
		function init() {
			...
			// setup options
			update_option('wploti_maintenance_redirect_version', '2.0.1');
		...
		*/ 
		...
		function wploti_enqueue_style_and_script_admin() {
			...
			wp_enqueue_style( 'select_slyle_src', $select_slyle_src, array(), null, 'all' );

			wp_enqueue_script( 'select-script', $select_script_src, array(), null, false );

			wp_enqueue_script( 'lottiplayer-script', $loti_script_src, array(), null, false );

			wp_enqueue_script( 'admin-script', $admin_script_src, array('lottiplayer-script','select-script'), null, false );
		...
		function extra_tab(){ 
			...
			<div class="wploti-tab-content">
				<table class="form-table"><tbody>
					...
					<tr class="description">
						<td  colspan="2"><p ><?php echo __('Selected users (when logged in) will <b>not</b> be affected by the maintenance mode and will always see the "normal" site.', 'maintenance-coming-soon-redirect-animation') ?> </p> </td>
					</tr>
